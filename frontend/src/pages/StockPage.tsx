...
export default function StockPage() {
    ...
    const poll = () => {
      stockService.getTip(ticker)
        .then((data: any) => {
          ...
          setLastUpdated(new Date())
          // If 1D chart, also refresh chart data
          if (period === '1d') {
  ...
  const rt = (term: string) => {
    ...
    const map: Record<string, string> = {
      ...
      'OVERBOUGHT' : 'May Be Overpriced ⚠',
      'OVERSOLD'   : 'May Be Undervalued 💡',
      'POSITIVE'   : 'Good News',
      'NEGATIVE'   : 'Bad News',
      'HIGH'       : 'High Risk',
      'MEDIUM'     : 'Medium Risk',
  ...
  const getHeadlineMeaning = (h: any): string => {
    if (h.label === 'Bullish')
      return isRookie
        ...
        : `Positive signal (compound: +${h.compound.toFixed(2)}). Bullish sentiment detected.`
    if (h.label === 'Bearish')
      return isRookie
        ? 'This is negative news. It may push the stock price lower.'
        : `Negative signal (compound: ${h.compound.toFixed(2)}). Bearish sentiment detected.`
    return isRookie
      ? "This news doesn't strongly push the stock price either way."
      : `Neutral signal (compound: ${h.compound.toFixed(2)}). No strong directional bias.`
  }
  ...
  const getOverallNewsImpact = (): string => {
    if (bullishPct > 55)
      return isRookie
        ...
        : `Sentiment skewed bullish (${bullishPct.toFixed(0)}% positive vs ${bearishPct.toFixed(0)}% negative). News adds upward pressure to the signal.`
    if (bearishPct > 55)
      return isRookie
    ...
    return isRookie
      ...
      : `Mixed sentiment (${bullishPct.toFixed(0)}% bull / ${bearishPct.toFixed(0)}% bear). News is a neutral factor in the overall signal.`
  }
  ...
  return (
    <>
      <style>{`
        ...
        @keyframes pricePulse{0%{opacity:1}50%{opacity:0.4}100%{opacity:1}}
      ...
      <div style={{ minHeight: '100vh', background: '#060d1f',
        ...
        <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '11px 32px', borderBottom: '1px solid rgba(59,130,246,0.1)', background: 'rgba(6,13,31,0.97)', backdropFilter: 'blur(20px)', position: 'sticky' as const, top: 0, zIndex: 100 }}>
          <button onClick={() => navigate('/dashboard')} style={{ background: 'transparent', border: 'none', color: '#64748b', fontFamily: 'monospace', fontSize: 12, cursor: 'pointer', letterSpacing: '0.06em' }}>
            ← DASHBOARD
          </button>
          ...
          <button onClick={() => navigate('/search')} style={{ background: '#0a1628', border: '1px solid rgba(59,130,246,0.2)', borderRadius: 3, padding: '6px 14px', cursor: 'pointer', color: '#94a3b8', fontFamily: 'monospace', fontSize: 11 }}>
            SEARCH →
          </button>
        </nav>
        ...
        <div style={{ maxWidth: 1320, margin: '0 auto', padding: '28px 32px' }}>
          ...
          {loading && (
            <div style={{ display: 'flex', flexDirection: 'column' as const, alignItems: 'center', justifyContent: 'center', minHeight: '60vh', gap: 16 }}>
              ...
              <div style={{ fontSize: 11, color: 'rgba(148,163,184,0.6)', fontFamily: 'monospace', letterSpacing: '0.1em' }}>LOADING {ticker}...</div>
            </div>
          )}
          ...
          {!loading && !error && analysis && (
            <div style={{ animation: 'fadeUp 0.5s ease both' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 24, paddingBottom: 24, borderBottom: '1px solid rgba(59,130,246,0.08)' }}>
                <div>
                  <div style={{ fontSize: 10, color: 'rgba(59,130,246,0.45)', fontFamily: 'monospace', letterSpacing: '0.12em', marginBottom: 8 }}>
                    SEC. ANALYSIS // {ticker}
                  </div>
                ...
                <div style={{ textAlign: 'right' as const }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 8, marginTop: 4 }}>
                    <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#22c55e', animation: 'pricePulse 2s ease-in-out infinite' }} />
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: 16, alignItems: 'start' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 14 }}>
                  ...
                  <div style={{ ...card, padding: 20 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                      <div style={{ fontSize: 9, color: 'rgba(99,102,241,0.6)', fontFamily: 'monospace', letterSpacing: '0.1em' }}>
                        ...
                        {chartLoading && <span style={{ color: 'rgba(148,163,184,0.6)', marginLeft: 8 }}>LOADING...</span>}
                      </div>
                    ...
                    <ResponsiveContainer width="100%" height={200}>
                      <AreaChart data={chartData.length > 0 ? chartData : []} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
                        <defs>
                        ...
                        />
                        <Tooltip
                          contentStyle={{ background: '#0a1628', border: '1px solid rgba(59,130,246,0.2)', borderRadius: 3, fontSize: 11, fontFamily: 'monospace', color: '#e2e8f0' }}
                  ...
                  <div style={{ ...card }}>
                    ...
                    <div style={{ padding: 20 }}>
                      ...
                      {tab === 'overview' && (
                        <div>
                          <div style={{ fontSize: 9, color: 'rgba(148,163,184,0.6)', fontFamily: 'monospace', letterSpacing: '0.1em', marginBottom: 14 }}>
                      ...
                      {tab === 'indicators' && (
                        <div>
                          <div style={{ fontSize: 9, color: 'rgba(148,163,184,0.6)', fontFamily: 'monospace', letterSpacing: '0.1em', marginBottom: 16 }}>// TECHNICAL INDICATOR READINGS</div>
                          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                            {[
                      ...
                      {tab === 'news' && (
                        <div>
                          <div style={{ fontSize: 9, color: 'rgba(148,163,184,0.6)', fontFamily: 'monospace', letterSpacing: '0.1em', marginBottom: 16 }}>// LIVE NEWS SENTIMENT ANALYSIS</div>
                                ...
                                <div key={i} style={{ background: '#060d1f', border: `1px solid ${h.label === 'Bullish' ? 'rgba(34,197,94,0.15)' : h.label === 'Bearish' ? 'rgba(239,68,68,0.15)' : 'rgba(59,130,246,0.07)'}`, borderRadius: 3, padding: '12px 14px' }}>
                                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                                    <span style={{ fontSize: 9, fontFamily: 'monospace', fontWeight: 700, color: h.label === 'Bullish' ? '#4ade80' : h.label === 'Bearish' ? '#f87171' : '#64748b' }}>
                                      {isRookie
                                        ? h.label === 'Bullish' ? '↑ GOOD NEWS'
                                          : h.label === 'Bearish' ? '↓ BAD NEWS'
                                          : '→ NEUTRAL'
                                        : h.label?.toUpperCase()}
                                    </span>
                                    <span style={{ fontSize: 9, color: '#334155', fontFamily: 'monospace' }}>
                                  ...
                                  <div style={{ background: 'rgba(59,130,246,0.04)', border: '1px solid rgba(59,130,246,0.08)', borderRadius: 2, padding: '7px 10px' }}>
                                    <span style={{ fontSize: 9, color: 'rgba(59,130,246,0.45)', fontFamily: 'monospace', letterSpacing: '0.06em' }}>
                                      {isRookie ? 'WHAT THIS MEANS: ' : 'SIGNAL: '}
                                    </span>
                                    <span style={{ fontSize: 11, color: '#64748b', fontFamily: 'monospace' }}>
                                      {getHeadlineMeaning(h)}
                                    </span>
                                  </div>
                                </div>
                              ))}
                      ...
                      {tab === 'backtest' && (
                        <div>
                          <div style={{ fontSize: 9, color: 'rgba(148,163,184,0.6)', fontFamily: 'monospace', letterSpacing: '0.1em', marginBottom: 16 }}>
                          ...
                          {backtest ? (
                            <>
                                ...
                                <div style={{ background: '#060d1f', border: '1px solid rgba(59,130,246,0.08)', borderRadius: 3, padding: '14px', marginBottom: 14 }}>
                                  ...
                                  <p style={{ fontSize: 12, color: '#94a3b8', fontFamily: 'monospace', lineHeight: 1.65, margin: 0 }}>
                                    {(backtest.strategy_return || 0) > (backtest.buyhold_return || 0)
                                      ? `Vestr's strategy would have made ${(backtest.strategy_return || 0).toFixed(1)}% over the last 5 years — better than simply holding the stock (${(backtest.buyhold_return || 0).toFixed(1)}%). This suggests the signals have real predictive value.`
                                      : `Simply holding the stock (${(backtest.buyhold_return || 0).toFixed(1)}%) would have outperformed the strategy (${(backtest.strategy_return || 0).toFixed(1)}%) over 5 years. Use the signals as guidance, not as guaranteed profit.`
                                    }
                                  </p>
                                </div>
                              )}
                              <div style={{ background: '#060d1f', border: '1px solid rgba(59,130,246,0.07)', borderRadius: 3, padding: '12px 14px' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                                  ...
                                  <span style={{ fontSize: 9, color: 'rgba(148,163,184,0.6)', fontFamily: 'monospace' }}>- - B&H</span>
                                </div>
                                <div style={{ height: 6, background: 'rgba(59,130,246,0.08)', borderRadius: 3, overflow: 'hidden', position: 'relative' as const }}>
                                  <div style={{ position: 'absolute' as const, left: 0, top: 0, bottom: 0, width: `${Math.min(Math.abs(backtest.strategy_return || 0), 100)}%`, background: 'linear-gradient(90deg, #6366f1, #22c55e)', borderRadius: 3 }} />
                                </div>
                            ...
                            <div style={{ fontSize: 12, color: '#475569', fontFamily: 'monospace' }}>LOADING BACKTEST DATA...</div>
                          )}
                        </div>
                      )}
                    </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 12, position: 'sticky' as const, top: 80 }}>
                  <div style={{ ...card, padding: 22 }}>
                    <div style={{ position: 'absolute' as const, top: 0, left: 0, right: 0, height: 2, background: `linear-gradient(90deg, ${verdictColor} 0%, transparent 60%)` }} />
                    <div style={{ position: 'relative' as const, overflow: 'hidden', background: '#060d1f', border: `1px solid ${verdictColor}18`, borderRadius: 3, padding: 20, marginBottom: 14, textAlign: 'center' as const }}>
                      ...
                      <div style={{ fontSize: 9, color: 'rgba(148,163,184,0.6)', fontFamily: 'monospace', letterSpacing: '0.14em', marginBottom: 10 }}>// VESTR VERDICT</div>
                  ...
                  {!showPortfolioForm ? (
                    ...
                    <div style={{ ...card, padding: 16 }}>
                      ...
                      <div style={{ marginBottom: 10 }}>
                        <div style={{ fontSize: 9, color: 'rgba(148,163,184,0.6)', fontFamily: 'monospace', marginBottom: 5 }}>SHARES</div>
                        <input value={shares} onChange={e => setShares(e.target.value)} type="number" placeholder="e.g. 10" style={inputStyle} />
                      </div>
                      <div style={{ marginBottom: 14 }}>
                        <div style={{ fontSize: 9, color: 'rgba(148,163,184,0.6)', fontFamily: 'monospace', marginBottom: 5 }}>AVG BUY PRICE ($)</div>
